...
const WaterfallSection: React.FC = () => {
  ...
  const saldo = total - redAdm; // 1.194.116,47
  const redJud = saldo; // Anulação total do restante
  
  // Cálculos de Porcentagem para altura das barras (base 100%)
  const pTotal = 100;
  ...
  const formatCurrency = (val: number) => {
    // Formatação compacta para caber melhor
        ...
        <ScrollReveal delay={0.2}>
            <GlassCard className="p-8 md:p-12 border-white/5 bg-neutral-900/40 backdrop-blur-3xl shadow-[0_0_50px_rgba(0,0,0,0.5)]">
                ...
                <div className="relative h-[400px] w-full flex items-end justify-between gap-2 md:gap-8 px-2 md:px-10">
                    ...
                    {/* Linhas de Grade de Fundo (Opcional) */}
                    ...
                    <div className="relative w-full h-full flex flex-col justify-end items-center group">
                        ...
                        <div 
                            style={{ height: `${pTotal}%` }} 
                            className="w-full md:w-24 bg-gradient-to-t from-neutral-800 to-neutral-600 rounded-t-lg relative hover:brightness-110 transition-all shadow-lg border-t border-white/10"
                        >
                        ...
                        {/* Linha Conectora para a próxima barra */}
                    ...
                    {/* Coluna 2: Redução Administrativa (Floating) */}
                    <div className="relative w-full h-full flex flex-col justify-end items-center group">
                        ...
                        {/* Spacer para fazer a barra "flutuar" do topo */}
                        <div style={{ height: `${pSaldo}%` }} className="w-full pointer-events-none" />
                        
                        <div 
                            style={{ height: `${pRedAdm}%` }} 
                            className="w-full md:w-24 bg-gradient-to-b from-red-500 to-red-700/80 rounded-lg relative hover:brightness-110 transition-all shadow-[0_0_20px_rgba(239,68,68,0.2)] border border-red-500/20 z-10"
                        >
                    ...
                    <div className="relative w-full h-full flex flex-col justify-end items-center group">
                        ...
                        <div 
                            style={{ height: `${pSaldo}%` }} 
                            className="w-full md:w-24 bg-gradient-to-t from-gold/20 to-gold/40 rounded-t-lg relative border-t border-gold/50 hover:bg-gold/30 transition-all backdrop-blur-sm"
                        >
                    ...
                    <div className="relative w-full h-full flex flex-col justify-end items-center group">
                        ...
                        {/* A barra vermelha aqui começa onde a barra de saldo parou (topo) e vai até zero */}
                        <div 
                            style={{ height: `${pRedJud}%` }} 
                            className="w-full md:w-24 bg-gradient-to-b from-red-500 to-red-700/80 rounded-lg relative hover:brightness-110 transition-all shadow-[0_0_20px_rgba(239,68,68,0.2)] border border-red-500/20"
                        >
                            <div className="absolute -top-6 left-1/2 -translate-x-1/2 text-red-400 font-bold text-xs whitespace-nowrap">
                                - {pRedJud.toFixed(0)}%
                            </div>
